...
func (q aurQuery) Less(i, j int) bool {
	...
	case "name":
		result = generic.LessRunes([]rune(q[i].Name), []rune(q[j].Name))
	case "base":
		result = generic.LessRunes([]rune(q[i].PackageBase), []rune(q[j].PackageBase))
	case "submitted":
...
func hangingPackages(removeOptional bool) (hanging []string, err error) {
	...
	provides := make(generic.MapStringSet)
...
func aurInfo(names []string, warnings *aurWarnings) ([]*rpc.Pkg, error) {
	...
	for n := 0; n < len(names); n += config.RequestSplitN {
		max := generic.Min(len(names), n+config.RequestSplitN)
		wg.Add(1)
