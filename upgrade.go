...
func (u upSlice) Less(i, j int) bool {
	if u[i].Repository == u[j].Repository {
		...
		return generic.LessRunes(iRunes, jRunes)
	}
...
func upgradePkgs(aurUp, repoUp upSlice) (generic.StringSet, generic.StringSet, error) {
	ignore := make(generic.StringSet)
	aurNames := make(generic.StringSet)
