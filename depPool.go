...
func (dp *depPool) ResolveTargets(pkgs []string) error {
	...
	// call
	aurTargets := make(generic.StringSet)
...
func (dp *depPool) findProvides(pkgs generic.StringSet) error {
	var mux sync.Mutex
...
func (dp *depPool) cacheAURPackages(_pkgs generic.StringSet) error {
	pkgs := _pkgs.Copy()
	query := make([]string, 0)
...
func (dp *depPool) resolveAURPackages(pkgs generic.StringSet, explicit bool) error {
	newPackages := make(generic.StringSet)
	newAURPackages := make(generic.StringSet)
